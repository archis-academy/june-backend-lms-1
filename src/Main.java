...
public class Main {
    ...
    public static void searchBook(String query) { // Search with bookTitle or bookId
        int temp = -1, i;
        ...
        for (i = 0; i < bookQuantity; i++) {
            if (books[i][2].equals(bookId)) {
                ...
                for (j = temp; j < bookQuantity; j++) {
                    booksNew[j][0] = books[i + 1][0];
                    booksNew[j][1] = books[i + 1][1];
                    booksNew[j][2] = books[i + 1][2];
                    booksNew[j][3] = books[i + 1][3];
                }
                books = booksNew;
